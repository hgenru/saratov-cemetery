...
define(
    ...
    function(jquery) {
        ...
        var IS_WORKER = !window.document, SCRIPT_PATH;
        ...
        jquery.fn.parse = function(options) {
                ...
                if (isFunction(options.before))
                {
                    ...
                    if (typeof returned === 'object')
                    {
                        ...
                        else if (returned.action === 'skip')
                        {
                            ...
                            f.instanceConfig = jquery.extend(f.instanceConfig, returned.config);
                    }
            ...
            this.each(function(idx) {
                var supported = jquery(this).prop('tagName').toUpperCase() === 'INPUT' &&
                                jquery(this).attr('type').toLowerCase() === 'file' &&
                                window.FileReader;
                ...
                for (var i = 0; i < this.files.length; i++)
                {
                    queue.push({
                        file: this.files[i],
                        inputElem: this,
                        instanceConfig: jquery.extend({}, config)
                    });
                }
            });
        ...
        if (IS_WORKER) {
            window.onmessage = workerThreadReceivedMessage;
        }
...
*/
    ...
    function newWorker()
    {
        if (!Papa.WORKERS_SUPPORTED)
            return false;
        var w = new window.Worker(SCRIPT_PATH);
    ...
    function isFunction(func)
    {
        ...
        Papa.WORKERS_SUPPORTED = !!window.Worker;
